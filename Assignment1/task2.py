...
class AlgorithmBatched:
    def __init__(self, num_arms, horizon, batch_size):
        ...
        self.index = [i for i in range(num_arms)]
        # END EDITING HERE
    
    def give_pull(self):
        # START EDITING HERE
        pulls = [0 for i in range(self.num_arms)]
        for i in range(self.batch_size):
            for j in range(self.num_arms):
                self.betaValues[j] = np.random.beta(self.successes[j] + 1, self.failures[j] + 1)
            pulls[np.argmax(self.betaValues)] += 1
        return self.index, pulls
        # END EDITING HERE
